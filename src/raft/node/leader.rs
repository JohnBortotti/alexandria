...
use super::super::message::Address::{Peer, Broadcast};
...
impl Role<Leader> {
    ...
    fn broadcast_heartbeat(self) -> Node {
        println!("broadcast_heartbeat, leader term is: {}", self.log.last_term);
        for peer in self.peers.iter() {
            self.node_tx.send(
                Message::new(
                    ...
                    Peer(self.id.clone()), 
                    Broadcast, 
                    AppendEntries{index:0,term: self.log.last_term}
                    )).unwrap();
            println!("peer: {}", peer);
        }
